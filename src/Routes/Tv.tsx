...
import { getPopularTv, getTv, IGetTvResult } from '../api';
...
function Tv() {
	...
	const popularMatch = useRouteMatch<{ tvId: string }>('/popularTv/:tvId');
	const { data: nowData, isLoading: nowLoading } = useQuery<IGetTvResult>(['tvs', 'top_rated'], getTv);
	const { data: popularData, isLoading: popularLoading } = useQuery<IGetTvResult>(['tvs', 'popularTv'], getPopularTv);
	...
	const onBoxClicked = (tvId: number, type: string) => {
		if (type === 'now') history.push(`/tv/${tvId}`);
		if (type === 'popular') history.push(`/popularTv/${tvId}`);
	};

	const clickedMovie = bigTvMatch?.params.tvId && nowData?.results.find(tv => tv.id === +bigTvMatch.params.tvId);

	const clickedPopularMovie =
		popularMatch?.params.tvId && popularData?.results.find(tv => tv.id === +popularMatch.params.tvId);
					...
					<AnimatePresence>
						{bigTvMatch ? (
							<>
								...
								<BigMovie
									...
									layoutId={`now_${bigTvMatch.params.tvId}`}
								>
									{clickedMovie && (
										<>
											<BigCover
						...
						{popularMatch ? (
							<>
								...
								<BigMovie
									style={{ top: scrollY.get() + 100 }}
									layoutId={`popular_${popularMatch.params.tvId}`}
								>
									{clickedPopularMovie && (
										<>
											<BigCover
											...
											/>
											<BigTitle>{clickedPopularMovie.name}</BigTitle>
